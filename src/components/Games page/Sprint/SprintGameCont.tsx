...
const SprintContainer = (props: {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (authData && props.level !== 7) {
        const res = await api.getAllUserWordsWithParams(randomPage, props.level - 1);
        ...
        if (filterWords.length < 20 && randomPage > 1) {
          ...
          const res = await api.getAllUserWordsWithParams((randomPage -= 1), props.level - 1);
