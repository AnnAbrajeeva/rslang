...
const ResultsTable: FC = () => {
  ...
  const { rightAnswers, wrongAnswers, bestGameStreak } = useTypedSelector(
    (state) => state.challenge
  );
  const rightAnswersPercent =
    rightAnswers.length + wrongAnswers.length > 0
      ? Math.round((rightAnswers.length / (rightAnswers.length + wrongAnswers.length)) * 100)
      : 0;
  ...
  useEffect(() => {
    if (user) {
      ...
      const newStatistic = prepareNewStatistic(
        prevStatistic,
  ...
  return (
    <StyledResultsTable>
      ...
      <StyledWrongAnswers>
        Errors - <span>{wrongAnswers.length}</span>
      </StyledWrongAnswers>
      {wrongAnswers.map((el) => (
        <ResultsItem key={el} index={el} />
      ))}
      <StyledRightAnswers>
        Correct - <span>{rightAnswers.length}</span>
      </StyledRightAnswers>
      {rightAnswers.map((el) => (
        <ResultsItem key={el} index={el} />
      ))}
