...
const ChallengeCard: FC = () => {
  ...
  const { currentQuestionsSet, currentQuestionIndex, currentAnswer, answers, isButtonsBlocked } =
    ...
    <StyledChallengeCard>
      <StyledQuestionWrapper>
        <MemoizedAudio
          count={count}
          setcount={setCount}
          src={currentQuestionsSet[currentQuestionIndex].audio}
        />
      ...
      {currentAnswer && <StyledButton onClick={handleNextClick}>Go</StyledButton>}
      {!currentAnswer && <StyledButton onClick={handleDontKnowClick}>Don&apos;t know</StyledButton>}
    </StyledChallengeCard>
  );
};
