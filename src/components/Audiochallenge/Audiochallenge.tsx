...
const Audiochallenge: FC = () => {
  const { isChallengeStarted, showResult, currentQuestionsSet } = useTypedSelector(
  ...
  const isShowLevel =
    !isStartedFromTextbook && !isChallengeStarted && !showResult && fetchAllWordsFulfilled;

  useEffect(() => {
    if (!user) {
      const startGame = async () => {
        await dispatch(fetchAllWords({ page: String(page), group: String(group) }));
        if (isStartedFromTextbook) {
          dispatch(setAnswersSet(allWords));
          const currentWords = shuffleArray(getWordsByPageAndGroup(allWords, group, page)).slice(
            NUM_OF_QUESTIONS
          );
  ...
  return (
    <Wrapper>
      ...
      {isChallengeStarted && !currentQuestionsSet.length && (
        <StyledAlert severity="info">
          Words for the game are not found. Restart the game.
        </StyledAlert>
      )}
      ...
      {fetchAllWordsFulfilled &&
        isChallengeStarted &&
        !showResult &&
        currentQuestionsSet.length && <ChallengeCard />}
