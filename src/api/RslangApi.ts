...
export default class RslangApi {
  ...
  constructor() {
    this.url = 'https://rs-lang-be-orcn.onrender.com/';
  }
  ...
  getWord = (id: string): Promise<IWord> => this.getResource(`${this.url}words/${id}`);

  // Создание пользователя
  createUser = (user: IUser): Promise<IUser> => this.postResource(`${this.url}users`, user);

  // Авторизация пользователя
  signInUser = (user: IUser): Promise<IAuthUser> => this.postResource(`${this.url}signin`, user);
